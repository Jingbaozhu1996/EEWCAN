...
%巴特沃斯滤波
function [v_ud,v_ew,v_ns]= raw_lvbo_vel(inputA,inputB,inputC,output2,dt,T)
...
t2=0:dt:t4;
v_ud=vA(nn1+1:nn1+t4/dt);%时间窗内的位移
v_ew=vB(nn2+1:nn2+t4/dt);%时间窗内的位移
v_ns=vC(nn3+1:nn3+t4/dt);%时间窗内的位移
