function [data,nt,t,ela,elo,sla,slo,M,sf,depth,Dir,Sta_H,Re,Maxacc,HD,RIQI,FZ_Time,JL_STime] = read_acc_data_zhenlitime(str)
fid  = fopen(str)
%本程序目的：仅读取P波数据
...
line1 = textscan(fid,'%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s',1)  %origin time
...
line10 = textscan(fid,'%s %s %s %s %s %s %s',1)  %Record Time 
...
line16 = textscan(fid,' %s %s %s %s %s %s %s %s %s',1) %Last Correction
