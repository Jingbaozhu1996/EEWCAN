%求震中距参数D,y=Et.根据caa
%巴特沃斯滤波
function [aas]= aas_distance(inputA,inputB,inputC,output2,dt,T,J)
...
%----------------4阶0.075HZ高通滤波-----------------------------------------%
inputA=filter(b,a,inputA);%滤波后的UD速度
inputB=filter(b,a,inputB);%滤波后的EW速度
inputC=filter(b,a,inputC);%滤波后的NS速度
...
k=t4/dt;
caaA=abs(inputA(n1:n1+k-1));%caa累积绝对加速度UD
caaB=abs(inputB(n1:n1+k-1));%caa累积绝对加速度UD
caaC=abs(inputC(n1:n1+k-1));%caa累积绝对加速度UD
hecheng=sqrt(((caaA).^2)+((caaB).^2)+((caaC).^2));%时间窗内三分向合成加速度
aas=sum(abs(hecheng));%aas累积绝对位移UD
