...
%    v1=v(n1:n1+i);%时间窗内的速度
    ...
    dew=dB(nn2:nn2+i);%时间窗内的位移
    dns=dC(nn3:nn3+i);%时间窗内的位移
    D_hecheng= sqrt(((dud).^2)+((dew).^2)+((dns).^2));%时间窗内三分向合成加速度
%    d1=d(n1:n1+i);%时间窗内的位移
    r1=trapz(t2,V_hecheng.^2);
    r2=trapz(t2,D_hecheng.^2);
    r=r1/r2;
    tc(i)=2*pi/sqrt(r);%特征周期
end
%------------计算tc参数波形---------%
...
%    v1=v(n1:n1+i);%时间窗内的速度
    ...
    dew=dB(nn2:nn2+i);%时间窗内的位移
    dns=dC(nn3:nn3+i);%时间窗内的位移
    D_hecheng= sqrt(((dud).^2)+((dew).^2)+((dns).^2));%时间窗内三分向合成位移
%    d1=d(n1:n1+i);%时间窗内的位移
    r1=trapz(t2,V_hecheng.^2);
    r2=trapz(t2,D_hecheng.^2);
    r=r1/r2;
...
k=t4/dt;
for i=1:1:k
    cadA=abs(vA(nn1:nn1+i));%cav累积绝对速度UD
    cadB=abs(vB(nn2:nn2+i));%cav累积绝对速度UD
    cadC=abs(vC(nn3:nn3+i));%cav累积绝对速度UD
    cad(i)= sum(sqrt(((cadA).^2)+((cadB).^2)+((cadC).^2)));%时间窗内三分向合成加速度
