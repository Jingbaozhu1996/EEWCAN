...
%巴特沃斯滤波
function [Pd]= Pd_lvbo_wave(inputA,inputB,inputC,output2,dt)
...
k=t4/dt;
for i=1:1:k
    dud=abs(dA(nn1:nn1+i));
    dew=abs(dB(nn2:nn2+i));
    dns=abs(dC(nn3:nn3+i));
    Pd(i)= max(sqrt(((dud).^2)+((dew).^2)+((dns).^2)));%时间窗内三分向合成加速度
end
%%%峰值位移的波形%%%%%
