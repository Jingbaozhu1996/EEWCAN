...
%巴特沃斯滤波
function [d1]= raw_lvbo_dis(inputA,inputB,inputC,output2,dt)
...
%----------------4阶0.075HZ高通滤波-----------------------------------------%
Fs = 1/dt;
fl= 0.15; % low cutoff frequency for highpass filter, depending on you
ftype = 'high';  %low
f_order = 4;% order
...
t2=0:dt:t4;
dud=abs(dA(nn1+1:nn1+t4/dt));%时间窗内的位移
dew=abs(dB(nn2+1:nn2+t4/dt));%时间窗内的位移
dns=abs(dC(nn3+1:nn3+t4/dt));%时间窗内的位移
d1= sqrt(((dud).^2)+((dew).^2)+((dns).^2));%时间窗内三分向合成位移
