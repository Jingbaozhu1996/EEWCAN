%求震中距参数D,y=Et.根据IV2
%巴特沃斯滤波
function [IV2]= IV2_distance(inputA,inputB,inputC,output2,dt,T,J)
...
v=v(n1:n1+t4/dt);%时间窗内的UD加速度
v1=v1(n1:n1+t4/dt);%时间窗内的EW加速度
v2=v2(n1:n1+t4/dt);%时间窗内的NS加速度


syn_vel= sqrt(((v).^2)+((v1).^2)+((v2).^2));%三分向合成加速度
IV2=max(cumtrapz(t2,syn_vel.^2));%速度平方积分，三分向的
